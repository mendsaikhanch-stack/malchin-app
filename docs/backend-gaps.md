...
Frontend:
- `services/owner-dashboard-data.ts` — pure data layer, 8 хэсэгт орсон
  aggregator + 5 health helper (`churnRatePct`, `dunningRatePct`,
  `moderationResolutionRate`, `coverageGapPct`,
  `contentReviewBacklogHours`) + `healthAlarms` (5 lock threshold).
- `app/owner.tsx` — Expo Web route (`/owner`). Платформ guard:
  Platform.OS === 'web' үед бүрэн dashboard, native үед "Веб админ
  зөвхөн" placeholder. **Mobile nav-аас холбохгүй (zero entry)** —
  CLAUDE.md §3 hewиглэсэн.
