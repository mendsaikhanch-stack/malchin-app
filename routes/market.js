...
router.put("/:id/status", (req, res) => { const item = listings.find(l => l.id == req.params.id); if (!item) return res.status(404).json({ error: "Зар олдсонгүй" }); item.status = req.body.status; res.json(item); });
